...
def test_profile_service_client_service_account_always_use_jwt(transport_class, transport_name):
...
@pytest.mark.parametrize("client_class", [
    ProfileServiceClient, ProfileServiceAsyncClient
])
@mock.patch.object(ProfileServiceClient, "DEFAULT_ENDPOINT", modify_default_endpoint(ProfileServiceClient))
@mock.patch.object(ProfileServiceAsyncClient, "DEFAULT_ENDPOINT", modify_default_endpoint(ProfileServiceAsyncClient))
...
@pytest.mark.parametrize("client_class,transport_class,transport_name,grpc_helpers", [
    (ProfileServiceClient, transports.ProfileServiceGrpcTransport, "grpc", grpc_helpers),
    (ProfileServiceAsyncClient, transports.ProfileServiceGrpcAsyncIOTransport, "grpc_asyncio", grpc_helpers_async),
])
def test_profile_service_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
...
def test_profile_service_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(
        credentials_file="credentials.json"
    )
...
async def test_list_profiles_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_profiles(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
async def test_search_profiles_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.search_profiles(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token

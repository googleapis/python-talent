...
class JobServiceClient(metaclass=JobServiceClientMeta):
    ...
    def purge_jobs(self,
            request: Union[job_service.PurgeJobsRequest, dict] = None,
            *,
            parent: str = None,
            filter: str = None,
            retry: OptionalRetry = gapic_v1.method.DEFAULT,
        ...
        r"""Purges all jobs associated with requested target.
        ...
        .. code-block:: python
            ...
            def sample_purge_jobs():
                # Create a client
                client = talent_v4.JobServiceClient()

                # Initialize request argument(s)
                request = talent_v4.PurgeJobsRequest(
                    parent="parent_value",
                    filter="filter_value",
                )

                # Make the request
                operation = client.purge_jobs(request=request)
        ...
        Args:
            request (Union[google.cloud.talent_v4.types.PurgeJobsRequest, dict]):
                The request object. Request to purge a set of jobs.
            parent (str):
                Required. The resource name of the project under which
                the jobs should be deleted.
            ...
            filter (str):
                ...
                1. Company. Resource name of the company under which all
                   the jobs should be deleted. The format is
                   "projects/{project_id}/tenants/{tenant_id}/companies/{company_id}".
                   For example, "projects/foo/tenants/bar/companies/baz"
                2. Tenant. Resource name of the tenant under which all
                   the jobs should be deleted. The format is
                   "projects/{project_id}/tenants/{tenant_id}". For
                   example, "projects/foo/tenants/bar".
                3. Project. Resource name of the project under which all
                   the jobs should be deleted. The format is
        ...
        Returns:
            google.api_core.operation.Operation:
                ...
                The result type for the operation will be :class:`google.cloud.talent_v4.types.PurgeJobsResponse` The result of [JobService.PurgeJobs][google.cloud.talent.v4.JobService.PurgeJobs]. It's used to
                   replace
                   [google.longrunning.Operation.response][google.longrunning.Operation.response]
                   in case of success.
        ...
        # Minor optimization to avoid making a copy if the user passes
        # in a job_service.PurgeJobsRequest.
        # There's no risk of modifying the input as we've already verified
        # there are no flattened fields.
        if not isinstance(request, job_service.PurgeJobsRequest):
            request = job_service.PurgeJobsRequest(request)
            # If we have keyword arguments corresponding to fields on the
            # request, apply these.
            if parent is not None:
                request.parent = parent
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.purge_jobs]
